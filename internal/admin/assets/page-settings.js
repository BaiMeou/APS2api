const SETTINGS_FIELDS = [
  ...
  { k: 'parallel_pool_retry_enabled', label: '单节点重试', type: 'bool', group: 'pool', desc: '开启：重试全部由节点内部完成（429/认证/5xx 原地重打），并禁用竞速接力换节点；关闭：单节点只打一次，由竞速池接力换节点完成重试' },
  ...
  { k: 'race_timeout', label: '单节点竞速超时 (秒)', type: 'number', max: 1800, min: 0, group: 'pool', desc: '单个节点在该时间内未返回首包即淘汰并切换下一个候选，防止慢/卡死节点拖累整轮重试 (0 = 不限制)' },
